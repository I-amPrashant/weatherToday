...
document.addEventListener('DOMContentLoaded', function () {
        ...
        const getCity=(city)=>{
            ...
            try {
              ...
              document.getElementById("cityName").innerHTML = `${result.location.name}, ${result.location.country}`;
              document.getElementById("current_temp_icon").src =
                result.current.condition.icon;
                document.getElementById("current-temp").innerHTML=`${result.current.temp_c} <sup>°</sup>`;
                const forecastDayOneELement=result.forecast.forecastday;
                const hourArray=forecastDayOneELement['0'];
                document.getElementById("min-temp").innerHTML=`${hourArray.day.mintemp_c} <sup>°</sup>`;
                    hourArray.hour.map((element)=>(
...
function displaySuggestions(suggestions) {
  document.getElementById("autocompleteContainer").innerHTML = '';
                suggestions.forEach(suggestion => {
                    ...
                    document.getElementById("autocompleteContainer").appendChild(suggestionElement)
